...
class SIPP
{
    static_assert(std::is_arithmetic<T>::value, "SIPP key type must be numeric.");
...
public:
    ...
    SIPP(double BUILD_LR_REMAIN = 0, bool QUIET = true)
    ...
    bool insert(const T& key, const P& value) {
        ...
        std::ifstream in("sipp_insert_root.log");
        if (!in.is_open()) {
            std::ofstream out("sipp_insert_root.log");
            out << "num_inserts,model_slope,model_intercept,num_slots" << std::endl;
        }
        std::ofstream out("sipp_insert_root.log", std::ios::app);
        ...
        // Node* cur_root = root;
        // if (cur_root != last_root) {
        // std::ofstream out("sipp_insert_root.log", std::ios::binary | std::ios::app);
    ...
    void print_depth_stats(std::string str) const {
        ...
        std::ofstream out_depth_dist("sipp_" + str + "_depth_distribution.log");
    ...
    void print_model_stats(std::string str) {
        ...
        std::ofstream out_file("sipp_" + str + "_model_stats.log");
        if (!out_file.is_open()) {
            std::cerr << "Failed to open file." << std::endl;
            return ;
        }
    ...
    void print_level_model_stats(std::string str) {
        std::ofstream out_file("sipp_" + str + "_level_model_stats.log");
        if (!out_file.is_open()) {
            std::cerr << "Failed to open file." << std::endl;
            return ;
        }
    ...
    void print_hist_model_stats(std::string str) {
        std::ofstream out_file("sipp_" + str + "_hist_model_stats.log");
        if (!out_file.is_open()) {
            std::cerr << "Failed to open file." << std::endl;
            return ;
        }
    ...
    void print_information_gain(std::string str) {
        ...
        std::ofstream hist_out_file("sipp_" + str + "_hist.log");
    ...
    void print_smo_stats(std::string s) {
        std::ofstream out("sipp_" + s + "_smo_stats.log");
        if (!out.is_open()) {
            std::cerr << "Failed to open file." << std::endl;
            return;
        }
    ...
    void print_node_size(std::string str) const {
        std::ofstream out_file("sipp_" + str + "_node_size.log");
        if (!out_file.is_open()) {
            std::cerr << "Failed to open file." << std::endl;
            return ;
        }
    ...
    void print_size_stats(std::string str) const {
        ...
        std::ofstream out("sipp_" + str + "_size_stats.log");
        if (!out.is_open()) {
            std::cerr << "Failed to open file." << std::endl;
            return;
        }
