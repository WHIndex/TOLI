...
public:
    ...
    bool add_point(const X &x, const Y &y) {
        ...
        if (outside_line1 || outside_line2) {
            // points_in_hull = 0;  // 改为在get_segment()中置0
            return false;
        }
...
size_t make_segmentation_at_bucket_level(size_t n, size_t epsilon, uint32_t block_size, Fin in, Fout out) {
    if (n == 0)
        return 0;
    ...
    for (size_t i = block_size; i < n; i += block_size) {
        p = in(i);
        if (prev_p.first == p.first) {
            continue;
        }
        if (!opt.add_point(p.first, p.second)) {
            size_t t = p.second - 1;
            while (i - block_size < t) {
                p = in(t);
                if (prev_p.first == p.first) {
                    break;
                }
