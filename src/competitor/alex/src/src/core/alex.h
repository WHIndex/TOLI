...
// node's parent.
 ...
 private:
  ...
  void expand_root(T key, bool expand_left) {
    ...
    //   // 这是不变式被破坏的信号，直接保护退出，避免把内存再写坏
    //   update_superroot_key_domain(); // 不会有副作用，但能修正 domain
    //   return;
    // }
    // printf("num_children_: %d\n",root->num_children_);
    // if (istats_.key_domain_min_ > istats_.key_domain_max_) {
    //   update_superroot_key_domain();
    // }
    ...
    // if (domain_size == 0) {
    //   // 最小扩张：强制按 2 倍扩；或先把 domain_size 设成 1
    //   domain_size = 1;
    // }
    ...
    if (expand_left) {
      ...
      if constexpr (std::is_integral<T>::value){
        ...
        // expansion_factor = pow_2_round_up((key_difference + domain_size - 1) / domain_size + 1);
        uint64_t inner = (static_cast<uint64_t>(key_difference) + domain_size - 1) / domain_size + 1;
        expansion_factor = static_cast<int>(pow_2_round_up_uint64(inner));  // Cast back if needed, but cap if > INT_MAX
        // if(expansion_factor <= 1){
        //   expansion_factor = 2;
        // }
        printf("1 key_difference: %lld, domain_size: %lld, expansion_factor: %d, key: %lld, istats_.key_domain_max_: %lld\n",
               static_cast<long long>(key_difference),
               static_cast<long long>(domain_size),
               expansion_factor, key, istats_.key_domain_max_);
      }
      else{
        ...
        expansion_factor = pow_2_round_up(static_cast<int>(
            std::ceil((key_difference + domain_size) / domain_size)));
        // printf("2key_difference: %.2f, domain_size: %.2f, expansion_factor: %d\n",
        //        key_difference,
    ...
    // printf(
    //     "Expanding root to the %s by factor %d. New domain: [%lld, %lld]\n",
    //     expand_left ? "left" : "right", expansion_factor,
    //     static_cast<long long>(new_domain_min),
    //     static_cast<long long>(new_domain_max));
    ...
    if (static_cast<size_t>(root->num_children_) * expansion_factor <=
      ...
      // printf(
      //     "Expanding root node %p from %d to %d children (expansion factor "
      //     "%d)\n",
      //     root, root->num_children_, root->num_children_ * expansion_factor,
      //     expansion_factor);
      ...
      // printf(
      //     "Creating new root node %p with %d children (expansion factor %d), "
      //     "old root %p becomes child\n",
      //     new_root, expansion_factor, expansion_factor, root);
    ...
    // if (n <= 0 || root->num_children_ <= 0) {
    //   // Invalid state: log and bail out to prevent crash
    //   printf("Invalid expansion: n=%d, num_children=%d, skipping\n", n, root->num_children_);
    //   update_superroot_key_domain();
    //   return;
    // }
    // printf(
    //     "Expanding root %p to %d children (expansion factor %d). "
    //     "Reassigning keys from outermost node %p. New domain: [%lld, %lld] n=%d\n",
    //     root, root->num_children_, expansion_factor, outermost_node,
    //     static_cast<long long>(new_domain_min),
    //     static_cast<long long>(new_domain_max), n);
    // assert(n > 0 && n <= root->num_children_);
    // int n = root->num_children_;
