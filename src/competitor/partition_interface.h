...
#define TALI_BOUND(a, x, b) ((x) < (a) ? (a) : ((x) > (b) ? (b) : (x)))
...
public:
  void prepare(std::string index_type, std::string partition_method,
    ...
    if (index_type == "alex") {
      ...
      for (size_t i = 0; i < partition_num; i++) {
        index_[i] = new BTreeHalfInterface<KEY_TYPE, PAYLOAD_TYPE>;
      }
      ...
      for (size_t i = 0; i < partition_num; i++) {
        index_[i] = new SIPPInterface<KEY_TYPE, PAYLOAD_TYPE>;
      }
    } else if (index_type == "vega") {
      for (size_t i = 0; i < partition_num; i++) {
        index_[i] = new VEGAInterface<KEY_TYPE, PAYLOAD_TYPE>;
      }
    } else if (index_type == "rs") {
      for (size_t i = 0; i < partition_num; i++) {
        index_[i] = new RSInterface<KEY_TYPE, PAYLOAD_TYPE>;
      }
    } else if (index_type == "rmi") {
      for (size_t i = 0; i < partition_num; i++) {
        index_[i] = new RMIInterface<KEY_TYPE, PAYLOAD_TYPE>;
      }
  ...
  void bulk_load(std::pair<KEY_TYPE, PAYLOAD_TYPE> *key_value, size_t num,
    ...
    if (!use_model_) {
      ...
      for (size_t i = 0; i < num; i++) {
        size_t predicted_idx = TALI_BOUND(
