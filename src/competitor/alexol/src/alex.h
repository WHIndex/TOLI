...
public:
  ...
  int range_scan_by_size(const T &key, uint32_t to_scan, V *&result) {
    EpochGuard guard;
    // RT_DEBUG("In range_scan_by_size. %d", key);
    if (result == nullptr) {
    ...
    // (Optimistic CC)
    // RT_DEBUG("before leaf range_scan_by_size. %d", key);
    // return leaf->range_scan_by_size(key, to_scan, result);
    int x = leaf->range_scan_by_size(key, to_scan, result);
    // RT_DEBUG("after leaf range_scan_by_size. %d", key);
    return x;
  }
  ...
  RETRY:
    ...
    if (fail) {
      ...
      if (fail == 5) { // Data node resizing
        ...
        // RT_DEBUG("fail=5 out insert. %d", key);
        return true;
      }
    ...
    // RT_DEBUG("final out insert. %d", key);
    return true;
  }
