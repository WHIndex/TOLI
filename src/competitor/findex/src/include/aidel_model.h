...
public:
    ...
    result_f con_find_retrain(const key_t &key, val_t &val);
    result_f update(const key_t &key, const val_t &val);
    inline bool con_insert(const key_t &key, const val_t &val);
    result_f con_insert_retrain(const key_t &key, const val_t &val);
    result_f remove(const key_t &key);
...
private:
    ...
    result_f insert_model_or_bin(const key_t &key, const val_t &val, size_t bin_pos);
    result_f remove_model_or_bin(const key_t &key, const int bin_pos);
