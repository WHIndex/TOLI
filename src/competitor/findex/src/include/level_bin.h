...
private:
    struct bin {
        ...
        void lock(){
            ...
            while (unlikely(cmpxchgb_f((uint8_t *)&this->locked, unlocked, locked) !=
                            unlocked))
              ;
        }
        void unlock(){
...
public:
    ...
    result_f con_find_retrain(const key_type &key, data_type &value) 
    {
        if(is_retraining) {
            return result_f::retrain;
        }

        uint64_t child_ver;
        child_bin* child = nullptr;
        result_f res = result_f::failed;
        memory_fence_f();
        if(m_root){
            root_bin* root = m_root;
...
private:
    ...
    // ========================= update the bins ==============================
public:
    result_f update(const key_type &key, const data_type &val)
    {
        if(is_retraining) {
            return result_f::retrain;
        }
...
public:
    ...
    inline result_f con_insert_retrain(const key_type& key, const data_type& data)
    {   
    ...
    {   
        // result_f result = con_insert_start(key, data);
        // while(result == result_f::retry) {
        //     result = con_insert_start(key, data);
        // }
        // if(result==result_f::ok) return true;
        // return false;
        result_f result = con_insert_start(key, data);
        if(result==result_f::ok) return true;
        return false;
...
private:
    result_f con_insert_start(const key_type& key, const data_type& value)
    {
        if(is_retraining){
            //COUT_THIS("level bin is retraining! "<<key);
            return result_f::retry;
        }
        memory_fence_f();
        if(m_root){
            //search the root
    ...
    result_f con_split_child_bin(child_bin* child, size_t slot, const key_type &key, const data_type &value)
    {
    ...
    result_f con_insert_start_retrain(const key_type& key, const data_type& value) 
    {
        if(is_retraining){
            COUT_THIS("level bin is retraining! "<<key);
            return result_f::failed;
        ...
        if(m_root) {
            ...
            if(slot > 0 && root->children[slot-1]->slotuse < rootslotmax) {
                ...
                result_f r = insert_child_retrain(root->children[slot], key, value, &newkey, &newchild);         
                if(newchild){
                    root->lock();
            ...
            child_bin *child = m_headbin;
            result_f r = insert_child_retrain(child, key, value, &newkey, &newchild);
            // create m_root
            if (newchild)
            {
    ...
    result_f insert_child_retrain(bin *b, const key_type& key, const data_type& value, 
...
public:
    result_f remove(const key_type &key)
    {
        if(is_retraining) {
            return result_f::retrain;
        }
