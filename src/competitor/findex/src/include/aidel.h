...
public:
    ...
    typedef typename findexInclude::OptimalPiecewiseLinearModel<key_t, size_t>::CanonicalSegment canonical_segment;
...
public:
    ...
    inline result_f find(const key_t &key, val_t &val);
    inline result_f insert(const key_t &key, const val_t &val);
    inline result_f update(const key_t &key, const val_t &val);
    inline result_f remove(const key_t &key);
