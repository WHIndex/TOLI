#ifndef __FINEDEX_UTIL_H__
#define __FINEDEX_UTIL_H__
...
inline void memory_fence_f() { asm volatile("mfence" : : : "memory"); }
...
inline void fence_f() { asm volatile("" : : : "memory"); }
 ...
 */
inline uint64_t cmpxchg_f(uint64_t *object, uint64_t expected,
                               uint64_t desired) {
...
inline uint8_t cmpxchgb_f(uint8_t *object, uint8_t expected,
                               uint8_t desired) {
...
FORCEINLINE uint32_t bsrr(uint32_t x) {
...
struct AtomicVal {
  ...
  void lock() {
    while (true) {
       ...
       *       else  != expected
       */
      if (likely(cmpxchg_f((uint64_t *)&this->status, expected, desired) ==
                 expected)) {
        return;
      }
    }
