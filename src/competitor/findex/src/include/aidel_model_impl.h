...
template<class key_t, class val_t>
size_t AidelModel<key_t, val_t>::size() const{
...
template<class key_t, class val_t>
result_f AidelModel<key_t, val_t>::con_find_retrain(const key_t &key, val_t &val)
{
    size_t pos = predict(key);
    pos = locate_in_levelbin(key, pos);
    if(key == keys[pos]){
...
// ======================= update =========================
template<class key_t, class val_t>
result_f AidelModel<key_t, val_t>::update(const key_t &key, const val_t &val)
{
    size_t pos = predict(key);
    pos = locate_in_levelbin(key, pos);
    if(key == keys[pos]){
...
template<class key_t, class val_t>
result_f AidelModel<key_t, val_t>::con_insert_retrain(const key_t &key, const val_t &val)
{
    size_t pos = predict(key);
    pos = locate_in_levelbin(key, pos);
...
template<class key_t, class val_t>
result_f AidelModel<key_t, val_t>::insert_model_or_bin(const key_t &key, const val_t &val, size_t bin_pos)
{
    // insert bin or model
    model_or_bin_t *mob = mobs[bin_pos];
    if(mob==nullptr){
...
// ========================== remove =====================
template<class key_t, class val_t>
result_f AidelModel<key_t, val_t>::remove(const key_t &key)
{
    size_t pos = predict(key);
    pos = locate_in_levelbin(key, pos);
    if(key == keys[pos]){
...
template<class key_t, class val_t>
result_f AidelModel<key_t, val_t>::remove_model_or_bin(const key_t &key, const int bin_pos)
{
    memory_fence_f();
    model_or_bin_t* mob = mobs[bin_pos];
    if(mob==nullptr) return result_f::failed;
