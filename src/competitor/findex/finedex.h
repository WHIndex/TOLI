...
public:
  ...
  long long memory_consumption() { return index->size(); }
...
bool finedexInterface<KEY_TYPE, PAYLOAD_TYPE>::get(KEY_TYPE key, PAYLOAD_TYPE &val, Param *param) {
  // finedex::result_f res = index.find(key, val);
  result_f res = index->find(key, val);
  if(res == result_f::ok) {
  // if(res == finedex::result_f::ok) {
    return true;
  }
  return false;
}
...
bool finedexInterface<KEY_TYPE, PAYLOAD_TYPE>::put(KEY_TYPE key, PAYLOAD_TYPE value, Param *param) {
  // finedex::result_f res = index.insert(key, value);
  result_f res = index->insert(key, value);
  if(res == result_f::ok) {
  // if(res == finedex::result_f::ok) {
    return true;
  }
...
bool finedexInterface<KEY_TYPE, PAYLOAD_TYPE>::update(KEY_TYPE key, PAYLOAD_TYPE value, Param *param) {
  // finedex::result_f res = index.update(key, value);
  result_f res = index->update(key, value);
  // if(res == finedex::result_f::ok) {
  if(res == result_f::ok) {
    return true;
  }
...
//  finedex::result_f res = index.remove(key);
  result_f res = index->remove(key);
  if(res == result_f::ok) {
  // if(res == finedex::result_f::ok) {
    return true;
  }
