...
class alexolcInterface final : public indexInterface<KEY_TYPE, PAYLOAD_TYPE> {
public:
    alexolcInterface() { std::signal(SIGTERM, handleSignal); }
...
private:
    alexolc::Alex <KEY_TYPE, PAYLOAD_TYPE, alexolc::AlexCompare, std::allocator<
...
template<class KEY_TYPE, class PAYLOAD_TYPE>
void alexolcInterface<KEY_TYPE, PAYLOAD_TYPE>::bulk_load(std::pair <KEY_TYPE, PAYLOAD_TYPE> *key_value, size_t num,
                                                        Param *param) {
...
template<class KEY_TYPE, class PAYLOAD_TYPE>
bool alexolcInterface<KEY_TYPE, PAYLOAD_TYPE>::get(KEY_TYPE key, PAYLOAD_TYPE &val, Param *param) {
    auto ret = index.get_payload(key, &val);
    return ret;
}

template<class KEY_TYPE, class PAYLOAD_TYPE>
bool alexolcInterface<KEY_TYPE, PAYLOAD_TYPE>::put(KEY_TYPE key, PAYLOAD_TYPE value, Param *param) {
    __sync_fetch_and_add(&alexolc_insert_succ, 1);
    return index.insert(key, value);
}

template<class KEY_TYPE, class PAYLOAD_TYPE>
bool alexolcInterface<KEY_TYPE, PAYLOAD_TYPE>::update(KEY_TYPE key, PAYLOAD_TYPE value, Param *param) {
    return index.update(key, value);
}

template<class KEY_TYPE, class PAYLOAD_TYPE>
bool alexolcInterface<KEY_TYPE, PAYLOAD_TYPE>::remove(KEY_TYPE key, Param *param) {
    int num = index.erase(key);
    if(num) return true; else return false;
}

template<class KEY_TYPE, class PAYLOAD_TYPE>
size_t alexolcInterface<KEY_TYPE, PAYLOAD_TYPE>::scan(KEY_TYPE key_low_bound, size_t key_num,
