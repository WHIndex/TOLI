...
template <typename KEY_TYPE, typename PAYLOAD_TYPE> class Benchmark {
  ...
  std::atomic<uint64_t> insert_count{0};  // 插入计数器（原子，确保多线程安全）
...
//           std::swap(operations, backup_operations);
//           std::swap(operations_num, backup_operations_num);
          // 新增：条件跳过 read 阶段（针对 test_suite=62）
          if ((test_suite != 62) && (test_suite != 61) ) {
          // if (test_suite != 62) {
            run(index);
          #ifdef PROFILING
            index->print_stats("read");
            ...
            // 只插后，直接 recover（避免 swap 混乱）
            std::swap(operations, backup_operations);
            std::swap(operations_num, backup_operations_num);
            COUT_THIS("Skipped read phase for test_suite=61,62 (insert only)");
          }
...
#pragma omp parallel for num_threads(thread_num)
    ...
    // For sequential reads, use sorted keys directly without shuffling or random sampling
    tbb::parallel_sort(keys, keys + table_size);  // Ensure keys are sorted for sequential order
    for (size_t i = 0; i < backup_operations_num; ++i) {
      backup_operations.push_back(
          std::pair<Operation, KEY_TYPE>(READ, keys[i]));
    }
  }
...
#pragma omp parallel for num_threads(thread_num)
    ...
    // step 3 backup_operations, backup_operations_num
    backup_operations_num = table_size;
    // backup_operations_num = 0; //这里不想要读 暂时写成这样 上一行是正确写法
    backup_operations.reserve(backup_operations_num);
    KEY_TYPE *sample_ptr = nullptr;
    std::shuffle(keys, keys + table_size, gen);
    if (sample_distribution == "uniform") {
...
#pragma omp parallel for num_threads(thread_num)
    ...
    // step 2 operations, operations_num
    // operations_num = table_size - init_table_size; // insert rest of all
    operations_num = 1000000000; // insert rest of all
    operations.reserve(operations_num);
    
    // 随机选取一个bulk过的键
    std::uniform_int_distribution<size_t> dist(0, init_keys.size() - 1);
    // KEY_TYPE selected_key = init_keys[dist(gen)];
    KEY_TYPE selected_key = 1000000000000;
    
    for (size_t i = 0; i < operations_num; ++i) {
      operations.push_back(std::pair<Operation, KEY_TYPE>(INSERT, selected_key+i));
    }
    // 由于所有插入键相同，排序无意义，可移除
    // tbb::parallel_sort(operations.begin(), operations.end()); // sorted insert
...
#pragma omp parallel for num_threads(thread_num)
    ...
    for (size_t i = 0; i < operations_num; ++i) {
      operations.push_back(std::pair<Operation, KEY_TYPE>(INSERT, selected_key));
    }
    // 由于所有插入键相同，排序无意义，可移除
    // tbb::parallel_sort(operations.begin(), operations.end()); // sorted insert
...
public:
  ...
  //   // ✅ 新增逻辑：输出排序后的前10个和后10个keys
  //   {
  //     std::vector<KEY_TYPE> sorted_keys(keys, keys + table_size);
  //     std::sort(sorted_keys.begin(), sorted_keys.end());

  //     std::cout << "Sorted keys (前10个): ";
  //     for (size_t j = 0; j < 10 && j < sorted_keys.size(); j++) {
  //         std::cout << sorted_keys[j] << " ";
  //     }
  //     std::cout << std::endl;

  //     std::cout << "Sorted keys (后10个): ";
  //     for (size_t j = (sorted_keys.size() > 10 ? sorted_keys.size() - 10 : 0); j < sorted_keys.size(); j++) {
  //         std::cout << sorted_keys[j] << " ";
  //     }
  //     std::cout << std::endl;
  // }
  ...
  void generate_operations(KEY_TYPE *keys) {
    ...
    if (sample_distribution == "uniform") {
      sample_ptr = get_search_keys(&init_keys[0], init_table_size,
                                   operations_num*(read_ratio + scan_ratio), &random_seed);
    } else if (sample_distribution == "zipf") {
    ...
    COUT_THIS("before generate operations.");
    ...
    for (size_t i = 0; i < operations_num; ++i) {
      ...
      if (prob < read_ratio) {
        ...
        // std::cout << "before generate scan operations sample_counter = " << sample_counter << std::endl;
        operations.push_back(
            std::pair<Operation, KEY_TYPE>(SCAN, sample_ptr[sample_counter++]));
        // std::cout << "after generate scan operations sample_counter = " << sample_counter << std::endl;
      } else {
...
#pragma omp for schedule(dynamic, 10000)
      for (auto i = 0; i < operations_num; i++) {
        ...
        if (op == READ) { // get
          ...
          // // 修改：添加计数和输出
          // static std::atomic<uint64_t> insert_count{0};
          // uint64_t current_count = insert_count.fetch_add(1) + 1;
          // printf("Inserted #%llu (key: %lu)\n", current_count, static_cast<unsigned long>(key));
          // fflush(stdout);
          // printf("Inserted #%llu (key: %lu)\n", i, static_cast<unsigned long>(key));
...
#pragma omp master
    ...
    print_stat();
    // printf("before delete thread_array\n");
    // delete[] thread_array;
    // printf("after delete thread_array\n");
  }
